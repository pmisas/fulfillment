package com.fulfillment.orderservice.resilience;
...
import com.fulfillment.orderservice.infrastructure.rest.GlobalExceptionHandler;

class OrderGlobalExceptionHandlerResilienceTest {
    ...
    @CsvSource({
        ...
        "redis,Redis idempotency store unavailable",
        "outbox,Outbox write failed",
        "json,Order event serialization failed"
    })
    ...
    private static class FailingDependencyController {
        ...
        void fail(@PathVariable String failure) {
            throw new RuntimeException(switch (failure) {
                ...
                case "redis" -> "Redis idempotency store unavailable";
                case "outbox" -> "Outbox write failed";
                case "json" -> "Order event serialization failed";
