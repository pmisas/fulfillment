...
public class OrderShippedHandler implements OrderEventHandler {
    ...
    private Mono<Void> applyShipped(Order order, String shipmentId) {
        ...
        OrderStateHistory history = OrderStateHistory.transition(order.getOrderId(), Status.PACKED, Status.SHIPPED);

        return transitionTx.transitionIfCurrentStatus(next, Status.PACKED, history)
            .flatMap(saved -> {
                if (!saved) {
                ...
                log.info("Order {} -> SHIPPED (shipmentId={})", order.getOrderId(), shipmentId);
                return Mono.empty();
            });
    }
